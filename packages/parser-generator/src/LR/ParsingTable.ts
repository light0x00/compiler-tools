	...
	for (let curState of stateSet) {
		...
		for (let curItem of curState) {
			...
			if (curItem.hasNext()) {
				...
				let existingOp = parsingTable.get(curState, followSymbol);
				...
				//shift
					...
					op = new Shift(nextState, curPrec, curLeftAssoc);
				debug(`state(${curState.id}) ${followSymbol} ${op}`);
				parsingTable.put(curState, followSymbol, op);
			}
			//归约项 形如: A->𝜶𝜷·
			else {
				...
				for (let followSymbol of allowedFollows) {
					...
					if (existingOp != null) { //issue #关于冲突检查
						if (existingOp.isReduce() && existingOp.prod.nonTerminal != curItem.prod.nonTerminal) { //issue #关于冲突检查 1. 归约-归约冲突
							if (!compareForReduceReduceConflict(
								{ prec: curPrec, leftAssoc: curLeftAssoc },
								{ prec: existingOp.prec, leftAssoc: curLeftAssoc })) {
								continue;
							}
						}
					...
					let op;
					//accept
					if (followSymbol == EOF && curItem.prod.nonTerminal == startSymbol)
						op = new Accept();
					//reduce
					else
						op = new Reduce(curItem.prod,curPrec,curLeftAssoc);
					debug(`state(${curState.id}) ${followSymbol} ${op}`);
					parsingTable.put(curState, followSymbol, op);
				}
			}
